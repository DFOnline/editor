...
export const ctxMenus: ContextMenu[] = [];
...
export default class ContextMenu {
    ...
    constructor(name: string, options: HTMLElement[], hasTitle = false) {
        ...
        events.forEach(e => this.HTMLElement[e] = (e: Event) => e.stopImmediatePropagation());
        this.HTMLElement.onmouseenter = () => this.isFocus = true;
        this.HTMLElement.onmouseleave = () => this.isFocus = false;
    ...
    close() {
        ...
        ctxMenus.splice(ctxMenus.findIndex(menu => menu.ref === this.ref), 1);
    }
    ...
    static setup() {
        ...
        else contextOverlay = document.querySelector(path)!;

        // close all when any of `events` is ran
        events.forEach(e => { document.body[e] = this.closeAll })
    }

    static closeAll() {
        ctxMenus.forEach(menu => menu.close())
    }
 ...
 */
function checkReady() {
    if (!contextOverlay) throw new Error('Make sure to run ContextMenu.setup() before using context menus.`');
}
